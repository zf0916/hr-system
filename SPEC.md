...
- **A screen showing a leave record says the count is the form's, and where it differs from the range, shows both numbers.** Not a discrepancy to reconcile: 1½ days over a two-day range is a half day, and a reader who sees only one of the two numbers will eventually correct the "wrong" one. Saying which number came from paper is what stops that. **The entry screen says it while the count is being typed**, beside the box, so the two numbers are seen together at the moment one of them is written down.
- **The entry screen's fields are in the order the paper prints them** — name, staff no., department, date of application, nature of leave, period from, to, no. of days — so a person reads down the screen and down the form together. The sheet legend's code is not one of them: it is not on the form, and it is asked for after the form's own fields are done.
...
|A51|**The guard roster is unread.** `screen_user` holds two placeholder guards, marked provisional, and the screen says on its face that the names are placeholders. **The HR side of the same table is not assumed**: Aisyah and Aslida are real names, not marked provisional, and the leave entry screen says nothing about them|
...
**HR's two are not part of that assumption and must not wear its warning.** A placeholder notice standing over a real name teaches a reader to ignore the notice where it is true, which is the whole value of A41 and A51. The rows carry no surname and no title, because neither has been read off anything and neither is needed to attribute a form somebody typed.
