...
class GatePassEntry(BaseModel):
    """What the gate pass entry screen may send, exhaustively.

    `extra="forbid"`, and here it carries two rules at once.

    **There is no `hours` field.** The hours are not written on the paper and
    are generated by the database from the two times, so a payload naming them
    is a `422` rather than a number quietly discarded — and if it were not, the
    service layer has no parameter for one and the column refuses an `INSERT`
    that names it (SPEC §5).

    **There is no `department` field either.** §5's form has none; the
    employee's section is looked up and never transcribed.

    The two times *are* here, because HR types them off the paper. That is not
    the guard entry path, which is server-stamped and has no time field at all
    (SPEC §3).
    """
...
def create_hr_app() -> FastAPI:
    ...
    # ---- gate pass entry (piece 5) ---------------------------------------

    @app.get("/api/gatepass/screen")
    def gate_pass_screen():
        """The form's field order, who may type it, and the four ticks."""
        with Session() as session:
            return JSONResponse(_read(gate_pass_entry.screen, session))

    @app.get("/api/gatepass/employee/{employee_number}")
    def gate_pass_look_up(employee_number: str, on: str | None = None):
        """The name, and the section that is looked up rather than typed."""
        with Session() as session:
            return JSONResponse(_read(gate_pass_entry.look_up, session,
                                      employee_number=employee_number, on=on))

    @app.post("/api/gatepass/entry")
    def gate_pass_record(entry: GatePassEntry):
        """One gate pass, through the same function `hr gatepass add` calls.

        **The hours are not in what goes in and are read back out of what the
        database stored.** There is no route that previews them either: a
        figure shown before the row exists would be a second place they are
        worked out, and §5 has only one (SPEC §5).
        """
        with Session() as session:
            return JSONResponse(_write(
                gate_pass_entry.record, session, entered_by=entry.entered_by,
                employee_number=entry.employee_number,
